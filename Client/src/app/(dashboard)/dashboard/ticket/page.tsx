              ...
              <tbody>
                {ticketList.map((ticket: any, index: number) => (
                  <tr key={index} className="border-b border-gray-100 hover:bg-gray-50">
                    <td className="p-4 text-sm text-gray-900">{ticket.department}</td>
                    <td className="p-4 text-sm text-gray-900">{ticket.subject}</td>
                    <td className="p-4 text-sm text-gray-600">{new Date(ticket.updated_at).toLocaleDateString()}</td>
                    <td className="p-4">
                      <div className={`bg-green-200 border-0 py-1 w-min rounded-md px-5 text-xs`}>{ticket.status}</div>
                    </td>
                  </tr>
                ))}
                ...
                {ticketList.length === 0 && (
                  <tr>
                    <td colSpan={4} className="p-4 text-center text-sm text-gray-600">
                      No tickets found
                    </td>
                  </tr>
                )}
