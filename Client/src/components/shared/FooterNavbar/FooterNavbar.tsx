...
const mobileBottomNavbar = [
  ...
  { icon: <HiOutlineSquares2X2 size={28} />, label: "Dashboard", path: "/categories" },
  { icon: <LuShoppingCart size={28} />, label: "Cart" },
  // { icon: <LiaFileInvoiceSolid />, label: "Invoice" },
  { icon: <IoPersonOutline size={28} />, label: "Profile", path: "/dashboard/settings" },
];
...
const FooterNavbar = () => {
  ...
  return (
    <div className="lg:hidden fixed bottom-0 flex items-center justify-around bg-white px-4 py-2 w-full z-50">
      {mobileBottomNavbar.map((item, index) => {
        ...
        return (
          <div
            ...
            className={cn("flex flex-col items-center rounded-full p-2", isActive ? "text-white bg-[#E5005F]" : "text-gray-500")}
          >
            {item.icon}
          </div>
        );
      ...
      <AuthModal open={isLoginModalOpen} onClose={() => setIsLoginModalOpen(false)} setIsPricingModalOpen={setIsPricingModalOpen} />
    </div>
  );
};
