...
const CartDrawer: React.FC<CartDrawerProps> = ({ isOpen, onClose }) => {
  ...
  const totalPrice = cartItems?.data?.reduce((total: number, item: any) => total + parseFloat(item.price) * item.qty, 0);

  return (
    <Drawer title="Your Cart" placement="right" closable={true} onClose={onClose} open={isOpen} width={350}>
        ...
        <div>
          {cartItems?.data?.length ? (
            <div className="space-y-4">
                ...
                <div key={item.id} className="flex items-center gap-3 border-b pb-3">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      {/* Delete Button */}
                      <button className="ml-auto text-red-500 cursor-pointer" onClick={() => handleDeleteCartItem(item.product_id)}>
              ...
              {/* Total Price */}
              <div className="pt-4 font-semibold text-lg">Total: ৳{totalPrice?.toFixed(2)}</div>
            </div>
          ) : (
