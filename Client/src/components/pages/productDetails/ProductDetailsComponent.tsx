...
async function getSingleProduct(productId: string) {
  const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/product-details/${productId}`, {
    method: "GET",
...
export default async function ProductDetailsComponent({ productId }: ProductDetailsComponentProps) {
