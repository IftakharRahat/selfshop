	...
	return (
		<div className={`w-full bg-white py-6 sm:py-8 lg:py-12 ${className}`}>
			<div className="container mx-auto px-4 sm:px-6 lg:px-8">
				{/* Section Title */}
				<h2 className="text-xl sm:text-2xl lg:text-3xl font-semibold text-[#322F35] mb-6 sm:mb-8">
					{title}
				</h2>
						...
						{featuredProducts.map((product: any, i: number) => (
							...
							>
								<div className="flex flex-col sm:flex-row">
									{/* Product Image */}
									<div
										className={` flex-shrink-0 w-full sm:w-64 h-48 sm:h-auto flex items-center justify-center p-6 `}
									>
									...
									<div className="flex-1 p-4 sm:pl-0 sm:p-6 flex flex-col justify-center">
										<Link href={`/product/${product?.ProductSlug}`}>
											<h3 className="text-base sm:text-lg font-medium text-gray-900 mb-3 line-clamp-2">
												{product?.ProductName}
											</h3>
										</Link>
				...
				{regularProducts.length > 0 && (
					<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3  gap-4 sm:gap-6">
						{regularProducts.map((product: any, i: number) => (
							<div
								key={i}
								className="bg-white rounded-lg border border-gray-200 overflow-hidden group cursor-pointer"
							>
								<div className="flex items-center">
									...
									<div className="flex-1 p-3 sm:p-4">
										<div className="flex items-center justify-between h-full">
											<Link href={`/product/${product?.ProductSlug}`}>
												<h3 className="text-gray-900 font-medium text-sm mb-2 line-clamp-1">
													{product?.ProductName}
												</h3>
											</Link>
