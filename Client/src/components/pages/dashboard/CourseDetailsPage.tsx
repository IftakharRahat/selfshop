...
export default function CourseDetailsPage() {
  ...
  if (isError || !data?.data) {
    return <div className="m-6 text-red-500">Failed to load course details.</div>;
  }
  ...
  return (
    <div className="m-4 lg:m-6 md:bg-white rounded-md md:p-6">
      ...
      {/* Course Info */}
      <h2 className="text-2xl font-bold mb-4">{courseCategory.coursecategory_name}</h2>
      {courses.length === 0 ? (
        <p className="text-gray-600">No courses available in this category yet.</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {courses.map((course: any) => (
            <div key={course.id} className="rounded-md shadow-md overflow-hidden">
              {course.image ? (
                <img
                  src={course.image}
                  alt={course.title}
