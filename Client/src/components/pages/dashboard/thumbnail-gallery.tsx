...
export default function ThumbnailGallery() {
	...
	if (isError) {
		return (
			<div className="m-6 text-red-500">Failed to load course categories.</div>
		);
	}
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
					{thumbnails.map((thumbnail: any) => (
						<div key={thumbnail.id}>
							<Link
								href={`/dashboard/free-course/${thumbnail.subtitle}`}
								className="block"
							>
								<div className="space-y-4">
									...
									<div className="relative overflow-hidden rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300">
										<div
											className={`relative h-48 ${thumbnail.bgColor} flex items-center justify-center`}
										>
											<img
												src={thumbnail.image}
												alt={thumbnail.title}
                  ...
                  Total Courses: {item.totalcourse || 0}
										...
										<p className="text-gray-600 text-sm leading-relaxed">
											{thumbnail.youtubeId
												? `Watch on YouTube: ${thumbnail.youtubeId}`
												: "No video available"}
										</p>
									</div>
								</div>
