...
function getTierForQty(tiers: Tier[], basePrice: number, qty: number): { price: number; label: string } {
  ...
  const price = tier.price >= 1 ? tier.price : Math.round(basePrice * tier.price);
...
type Props = {
  ...
  tiers?: Tier[];
  onConfirm: (payload: { variantQuantities: Record<string, number>; totalQty: number; unitPrice: number; total: number }) => void;
  onClose?: () => void;
};
...
export default function BulkOrderMatrix({
  ...
  const [quantities, setQuantities] = React.useState<Record<string, number>>(initialQty);
      ...
      <div className="flex gap-4 mb-6 p-4 bg-white rounded-xl border border-gray-100">
        {productImage && (
          <div className="w-20 h-20 rounded-lg overflow-hidden flex-shrink-0 bg-gray-100">
            <Image src={getImageUrl(productImage)} alt={productName} width={80} height={80} className="w-full h-full object-cover" />
          </div>
        )}
        <div>
          ...
          <div className="flex justify-between items-center">
            ...
            <span>
              ...
              <span className="font-medium">
                <TbCurrencyTaka className="inline" />
                {unitPrice.toFixed(0)} {totalQty > 0 && `(${tierInfo.label} applied)`}
              </span>
            </span>
          </div>
