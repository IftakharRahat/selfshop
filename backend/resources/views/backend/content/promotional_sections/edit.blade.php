        ...
        <div class="col-12">
            <div class="admin-content-card">
                ...
                <div class="admin-card-body">
                    <form action="{{ route('admin.promotional-sections.update', $section->id) }}" method="POST" enctype="multipart/form-data" id="sectionForm">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-lg-4">
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <select name="layout_type" id="layoutType" class="form-select" onchange="toggleLayoutFields()">
                                        <option value="card" {{ old('layout_type', $section->layout_type) == 'card' ? 'selected' : '' }}>Card (Banner + Product Grid)</option>
                                        <option value="slider" {{ old('layout_type', $section->layout_type) == 'slider' ? 'selected' : '' }}>Slider (Swiper Product Row)</option>
                                    </select>
                                ...
                                <div class="form-group mb-3" id="bannerField">
                                    ...
                                    @if($section->banner_image)
                                        <div class="current-banner-wrapper" id="currentBannerWrap">
                                            <img src="{{ $section->banner_image }}" alt="Current Banner" id="currentBanner">
                                            <div class="current-banner-label">Current banner</div>
                                        </div>
                                    @endif
                                ...
                                <div class="form-group mb-3" id="bgColorField" style="display:none;">
                                    ...
                                    <div class="d-flex align-items-center gap-2">
                                        <input type="color" name="bg_color" id="bgColorPicker" value="{{ old('bg_color', $section->bg_color ?? '#ffffff') }}"
                                            class="color-preview" oninput="document.getElementById('bgColorHex').value = this.value">
                                        <input type="text" id="bgColorHex" class="form-control" value="{{ old('bg_color', $section->bg_color ?? '#ffffff') }}" style="max-width:120px;"
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <input type="number" name="sort_order" class="form-control" value="{{ old('sort_order', $section->sort_order) }}" min="0" placeholder="0">
                            ...
                            <div class="col-lg-8">
                                <div class="form-group mb-3">
                                    <label class="form-label fw-semibold">
                                        Assign Products
                                        <span class="product-count-badge ms-2" id="selectedCount">{{ count($selectedProductIds) }} selected</span>
                                    </label>
                                    ...
                                    <div class="product-grid" id="productGrid">
                                        @foreach($products as $product)
                                            ...
                                            @endphp
                                            <div class="product-grid-item {{ $isSelected ? 'selected' : '' }}"
                                                 data-id="{{ $product->id }}"
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        document.querySelectorAll('.product-grid-item.selected').forEach(function(el) {
            selectedProducts[el.dataset.id] = { name: el.dataset.name, img: el.dataset.img };
        });
        renderSelectedTags();
        renderHiddenInputs();
        toggleLayoutFields();
    ...
    function previewBanner(event) {
        ...
        var cur = document.getElementById('currentBannerWrap');
        if (cur) cur.style.opacity = '0.3';
