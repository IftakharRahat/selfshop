...
return new class extends Migration
{
    public function up(): void
    {
        ...
        // 1. Create promotional_sections table (skip if already exists)
        if (!Schema::hasTable('promotional_sections')) {
        ...
        // 2. Create pivot table (skip if already exists)
        if (!Schema::hasTable('promotional_section_products')) {
        ...
        // 3. Seed only if we just created the table
        if ($freshlyCreated) {
            $sections = [
